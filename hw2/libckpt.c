...
void save_memory(int fd, struct ckpt_header proc_maps[]);
...
int match_one_line(int proc_maps_fd,
                   struct ckpt_header *proc_maps_line, char *filename) {
...
int proc_self_maps(struct ckpt_header proc_maps[]) {
...
void save_memory(int fd, struct ckpt_header proc_maps[]) {
  ...
  if(is_restart == 1) {
    is_restart = 0;
    printf("Restarted and returning from signal_handler!\n");
    return;
  }
  is_restart = 1;
  ...
  strncpy(context_header.name,
            "REGISTER CONTEXT", strlen("REGISTER CONTEXT")+1);
  write_helper(fd, &context_header, sizeof(struct ckpt_header));
  write_helper(fd, &context, sizeof(ucontext_t));
  int i = 0;
  for (i = 0; proc_maps[i].start != NULL; i++) {
    write_helper(fd, &proc_maps[i], sizeof(struct ckpt_header));
